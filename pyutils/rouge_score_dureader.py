...
def get_dureader_sccore(pred_file, ref_file, task='main'):
    """
    计算dureader 得分，传入预测与参考文件地址，默认task为main
    'task name: Main|Yes_No|All|Entity|Description'
    :param pred_file:
    :param ref_file:
    :param task: Main|Yes_No|All|Entity|Description
    :return: 输出得分
    -------
    文件的单条数据样例，json格式，每行为一条。
    每行关键字如下，ref_file文件包含source，'search'或者 'zhidao'
    example:
            key:  question_id  value_len:  6  type:  <class 'str'>
            key:  question_type  value_len:  6  type:  <class 'str'>
            key:  entity_answers  value_len:  4  type:  <class 'list'>
            key:  yesno_answers  value_len:  1  type:  <class 'list'>
            key:  answers  value_len:  1  type:  <class 'list'>
            key:  source  value_len:  6  type:  <class 'str'>
    """
    err = None
    metrics = {}
    try:
        pred_result = read_file(pred_file, task)
        ref_result = read_file(ref_file, task, is_ref=True)
        # 数据来源： 百度搜索或者百度知道
        sources = ['both', 'search', 'zhidao']
    ...
    print(json.dumps(
            format_metrics(metrics, task, err),
